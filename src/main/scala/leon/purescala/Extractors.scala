...
object Extractors {
  ...
  object UnaryOperator {
    def unapply(expr: Expr) : Option[(Expr,(Expr)=>Expr)] = expr match {
      ...
      case SelectDynamic(rec, name) => Some((rec, SelectDynamic(_, name)))
  ...
  object NAryOperator {
    def unapply(expr: Expr) : Option[(Seq[Expr],(Seq[Expr])=>Expr)] = expr match {
      ...
      case ad @ ApplyDynamic(rec, name, args) => Some((rec +: args, as => ApplyDynamic(as.head, name, as.tail).setPos(ad)))
